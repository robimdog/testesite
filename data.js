...
/* ===== Conteúdo editável (via painel /admin) =====
   Lê content/products.json — editado pelo painel de administração,
   assim o cliente publica/edita produtos sem mexer em código.
   Se o arquivo não for encontrado (ex: abrindo o HTML localmente,
   sem publicar), o site segue funcionando com o catálogo padrão acima. */
  ...
  try{
    ...
    if(res.ok){
      ...
      if(data.items && data.items.length){
        products = data.items.map((p, i) => ({...p, id: i + 1}));
      }
    }
  }catch(err){
    console.warn('Catálogo editável não carregado (normal ao abrir o arquivo localmente). Usando catálogo padrão.', err);
  }
}
